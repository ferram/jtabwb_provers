...
```
java -jar eick4.jar -i   # use the -i option to read the formula from standard input (see the syntax below)
java -jar eick4.jar file # the file must specify the formula in the JTabWb format    (see the syntax below)
```

The directory   [examples](https://github.com/ferram/jtabwb_provers/tree/master/ick4_eick4/examples)  contains some problems in the JTabWb format.
For instance, the command

```
java -jar eick4.jar file examples/ick4_axiom_c_completeness.jtabwb  
```
...
```
java -jar eick4.jar -latex examples/ick4_axiom_c_completeness.jtabwb        #  yields a derivation
java -jar eick4.jar -latex examples/ick4_unprovable_godel_lob_axiom.jtabwb  #  yields all the open proof-trees           
```

Executing `eick4`  with the `-model` option, the prover searches for a proof in the refutation calculus and saves the LaTeX of the generated countermodel, e.g.,


```
java -jar eick4.jar -model examples/ick4_unprovable_godel_lob_axiom.jtabwb #  yields the counter model
```
